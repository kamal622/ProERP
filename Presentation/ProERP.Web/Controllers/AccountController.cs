...
namespace ProERP.Web.Controllers
{
    ...
    public class AccountController : BaseController
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    if (HttpContext.User.IsInRole("SysAdmin"))
                        ...
                        result = await UserManager.AddToRoleAsync(user.Id, "Lavel2");
                    else if (HttpContext.User.IsInRole("Lavel2"))
                        result = await UserManager.AddToRoleAsync(user.Id, "Lavel1");
                    else
                        result = await UserManager.AddToRoleAsync(user.Id, "Lavel3");
                    if (result.Succeeded)
                    {
        ...
        public JsonResult GetAllUsersList(string UserName, string Email)
        {
            var allData = this._userService.GetAllUsers(UserName, Email);
            var Users = allData.Select(a => new
            {
                a.Id,
                a.UserName,
                a.Email

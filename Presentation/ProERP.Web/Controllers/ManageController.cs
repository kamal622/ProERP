...
namespace ProERP.Web.Controllers
{
    ...
    public class ManageController : BaseController
    {
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ...
            string firstName = "";
            string lastName = "";
            this._userService.GetUsersFnameLname(userId, out firstName, out lastName);
        ...
        [HttpGet]
        public JsonResult GetUsersList(string UserName, string Email)
        {
            var allUserData = this._userService.GetAllUsers(UserName, Email);
            var Users = allUserData.Select(a => new
            {
                a.Id,
                a.UserProfile_Id,
                a.UserName,
        ...
        public JsonNetResult UpdateUserData(int Id, bool IsActive, bool IsBlocked)
        {
            var result = new JsonResponse { Status = JsonResponseStatus.Success, Message = "User added successfully." };
            try
            {
                this._userService.UpdateUserData(Id, IsActive, IsBlocked);
        ...
        public JsonNetResult UpdatePassword(string NewPassword, int userId)
        {
            JsonResponse response = new JsonResponse();
            try
            {
                var tokens = UserManager.GeneratePasswordResetToken(userId);
                var result = UserManager.ResetPassword(userId, tokens, NewPassword);
                ...
                {                    
                    ...
                    response.Message = string.Join("</br>", result.Errors);
                }
            }
            catch(Exception e)
            {
        ...
        [HttpPost]
        public JsonNetResult AddUser(string roleName, string uname, string email, string pass, string fname, string lname)
        {
            var result = new JsonResponse { Status = JsonResponseStatus.Success, Message = "User added successfully." };
            try
            {
                var user = new User { UserName = uname, Email = email, EmailConfirmed = true };
                ...
                var r = UserManager.Create(user, pass);
                ...
                {
                    result.Status = JsonResponseStatus.Error;
                    result.Message = String.Join("", r.Errors);
                }
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonNetResult UpdateUser(string fname, string lname, string email)
        {
            JsonResponse response = new JsonResponse();
            ...
            try
            {
                ...
                var errorCode = this._userService.Update(fname, lname, email, userId);
                ...
                {
                    response.Status = JsonResponseStatus.Error;
                    response.Message = "Email already exists. Profile not updated";
                }
            }
            catch (Exception ex)
            {
        ...
        {
            var allUserData = this._userService.GetAllUserRole("");
            var Roles = allUserData.Select(a => new
            {
                a.Id,
                a.Name
            });
        ...
        //[HttpPost]
        //public ActionResult ManageUsers(ManageUsersViewModel model)
        //{
        //    return View();
        //}

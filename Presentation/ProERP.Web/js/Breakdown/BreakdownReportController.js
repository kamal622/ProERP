ProApp.controller("BreakdownsReportController", function ($scope, $http) {
    $("#lblError").text("");
    ...
    var LoadIframe = function () {
        ...
        {
            var machineId = 0;//$scope.ddlMachine.val();
            ...
            if (selection.from == null || selection.to == null)
                ...
                $("#lblError").text("");
                var url = "/Reports/ReportTemplate.aspx?SiteId=1&PlantId=" + plantId + "&LineId=" + lineId + "&MachineId=" + machineId + "&StartDate=" + selection.from.toISOString() + "&EndDate=" + selection.to.toISOString();
